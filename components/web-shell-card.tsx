...
import type { WebShell } from "@/lib/data"
...
export default function WebShellCard({ webShell }: WebShellCardProps) {
  ...
  const copyToClipboard = () => {
    navigator.clipboard.writeText(webShell.url)
    setCopied(true)
  ...
  return (
    <div className="bg-[#1a1a1a] border border-[#2a2a3a] rounded-lg overflow-hidden hover:border-[#00ff9d]/50 transition-all duration-300 group">
      <div className="p-6">
        <div className="flex items-start justify-between mb-4">
          <div className="flex items-center">
            <div className="mr-3 p-2 bg-[#2a2a3a] rounded-md">
              <FileCode className={`h-5 w-5 ${getTypeColor(webShell.type)}`} />
            </div>
            <div>
              <h3 className="text-lg font-bold font-mono group-hover:text-[#00ff9d] transition-colors">
                {webShell.name}
              </h3>
              <p className="text-sm text-gray-400 font-mono">{webShell.type.toUpperCase()}</p>
            </div>
          </div>
        ...
        <p className="text-gray-300 mb-4 font-mono text-sm">{webShell.description}</p>
      ...
      <div className="px-6 py-4 bg-[#0d0d0f] border-t border-[#2a2a3a] flex justify-between items-center">
        <span className="text-xs text-gray-400 font-mono">Last updated: {webShell.lastUpdated}</span>
